...
public:
    ...
    virtual void Forward2d(int n1, int n2, const ComplT* in, ComplT* out, unsigned options = 0) {
        Transform2d(n1, n2, -1, in, out, options);
    }

    // Inverse Fourier transform of 2d complex data written in a C-style array
    virtual void Inverse2d(int n1, int n2, const ComplT* in, ComplT* out, unsigned options = 0) {
        Transform2d(n1, n2, 1, in, out, options);
